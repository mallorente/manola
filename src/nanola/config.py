...
class AppConfig(BaseModel):
    workspace_dir: Path = Path.home() / "Nanola" / "Meetings"
    shared_dir: Path | None = None
    models_dir: Path = Path.home() / ".nanola" / "models"
...
def write_secrets_template(*, overwrite: bool = False) -> Path:
    ...
    SECRETS_PATH.write_text(
        "\n".join(
            [
                "# Nanola secrets. Prefer environment variables when possible.",
                '# OPENROUTER_API_KEY = "..."',
                '# OPENAI_API_KEY = "..."',
                '# WHISPER_API_KEY = "..."',
                "",
...
def update_config_value(name: str, value: str | Path | bool) -> Path:
    config = load_config()
