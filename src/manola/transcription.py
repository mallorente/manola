...
from .errors import ConfigurationError, DependencyMissingError, ManolaError
...
def _transcribe_local(
    ...
    try:
        ...
        if (
            ...
            raise ManolaError(_local_transcription_error(exc, config)) from exc
    ...
    try:
        ...
        raise ManolaError(f"Local transcription failed on CPU fallback: {exc}") from exc
...
def _transcribe_with_model(
    ...
    with tempfile.TemporaryDirectory(prefix="manola-whisper-") as temp_dir:
...
def _extract_audio_chunk(source: Path, target: Path, *, start: int, duration: int) -> None:
    ...
    if completed.returncode != 0:
        ...
        raise ManolaError(f"FFmpeg failed to extract audio chunk: {details}")
...
def _transcribe_local_in_worker(
    ...
    with tempfile.TemporaryDirectory(prefix="manola-worker-") as temp_dir:
        config_path = Path(temp_dir) / "config.json"
        output_path = Path(temp_dir) / "transcript.txt"
        config_path.write_text(config.model_dump_json(), encoding="utf-8")
        env = dict(**__import__("os").environ)
        env["MANOLA_TRANSCRIBE_WORKER"] = "1"
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "manola.transcribe_worker",
                str(audio_path),
                str(output_path),
                "--language",
                language.value,
        ...
        if completed.returncode != 0:
            details = completed.stderr.strip() or completed.stdout.strip()
            raise ManolaError(f"CUDA transcription worker failed: {details}")
        return output_path.read_text(encoding="utf-8").strip()


def _running_in_worker() -> bool:
    return __import__("os").environ.get("MANOLA_TRANSCRIBE_WORKER") == "1"
...
def _local_transcription_error(exc: RuntimeError, config: AppConfig) -> str:
    ...
    return (
        ...
        "Run `manola doctor` to check CUDA dependencies, or set "
...
def _transcribe_remote(audio_path: Path, language: Language, config: AppConfig) -> str:
    if not config.remote_transcription_url:
        raise ConfigurationError(
            "Remote transcription requires remote_transcription_url in ~/.manola/config.toml."
        )
    ...
    if response.status_code >= 400:
        raise ManolaError(f"Remote transcription failed: {response.status_code} {response.text}")

    payload = response.json()
    text = payload.get("text")
    if not isinstance(text, str) or not text.strip():
        raise ManolaError("Remote transcription response did not contain a non-empty 'text' field.")
    return text.strip()
